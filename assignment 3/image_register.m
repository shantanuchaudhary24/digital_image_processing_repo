...
function [] = image_register(img1, img2)
    ...
    % The procedure is as follows (note thiistratrgets does not compute scale)
    ...
    IA = hipass_filter(size(img1, 1),size(img1,2)).*abs(FFT1);  
    IB = hipass_filter(size(img2, 1),size(img2,2)).*abs(FFT2);  
    ...
    L1 = transformImage(IA, X_Size, Y_Size, X_Size, Y_Size, 'nearest', size(IA) / 2, 'valid');
    L2 = transformImage(IB, X_Size, Y_Size, X_Size, Y_Size, 'nearest', size(IB) / 2, 'valid');
    ...
    total_height = max(size(img1,1),(abs(move_ht)+size(input2_rectified,1)));
    total_width =  max(size(img1,2),(abs(move_wd)+size(input2_rectified,2)));
    ...
    if((move_ht>=0)&&(move_wd>=0))
        registered1(1:size(img1,1),1:size(img1,2)) = img1;
    ...
    elseif ((move_ht<0)&&(move_wd<0))   % if translations are both NEGATIVE
        ...
        registered1((1+abs(move_ht)):(abs(move_ht)+size(img1,1)),(1+abs(move_wd)):(abs(move_wd)+size(img1,2))) = img1;
    elseif ((move_ht>=0)&&(move_wd<0))
        ...
        registered1(1:size(img1,1),(abs(move_wd)+1):(abs(move_wd)+size(img1,2))) = img1;
    elseif ((move_ht<0)&&(move_wd>=0))
        registered1((abs(move_ht)+1):(abs(move_ht)+size(img1,1)),1:size(img1,2)) = img1;
